...
def cifar_noniid_unequal(dataset, num_users):
    ...
    number of training imgs
    """
    # Dynamically calculate the number of shards and images per shard
    num_imgs = 50  # Fixed size per shard
    num_shards = len(dataset) // num_imgs  # Ensure it matches the dataset size
    ...
    # Truncate idxs to match the number of labels, if needed
    idxs = idxs[:len(labels)]
    ...
    if sum(random_shard_size) > num_shards:
        for i in range(num_users):
            # Ensure every client gets at least one shard
            rand_set = set(np.random.choice(idx_shard, 1, replace=False))
            idx_shard = list(set(idx_shard) - rand_set)
            for rand in rand_set:
                dict_users[i] = np.concatenate(
        ...
        # Assign leftover shards to the client with the least data
        if len(idx_shard) > 0:
            shard_size = len(idx_shard)
