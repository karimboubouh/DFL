...
def exp_details(args):
    ...
    log('log', f'Used ML engine: {ML_ENGINE}')
...
def args_parser():
    ...
    parser.add_argument('--optimize_network', action='store_true', help="Optimize the network graph W")
...
def grads_norm_squared(vi, vj):
    squared_norm = 0.0
    for key in vi.keys():
        if vi[key] is not None and vj[key] is not None:
            squared_norm += ((vi[key] - vj[key]) ** 2).sum().item()
    return squared_norm


def convergence_strategy(t, rounds, strategy="SD"):
    n = t / rounds
    if strategy == "SD":
        return n
    elif strategy == "DS":
        return 1 - n  # Linearly decreases from 1 to 0
    elif strategy == "SDS":
        if n <= 0.5:
            return 2 * n
        else:
...
def density_index(W):
    """
    Compute the density index of a given matrix W.
    """
    total_elements = W.size  # Total number of elements in the matrix
