...
class Node(Thread):
    ...
    def neighborhood_data_scope(self):
        ...
        for neighbor in self.neighbors:
            scope = scope.union(neighbor.node.local_data_scope())
        return list(scope)
...
class NodeConnection(Thread):
    ...
    def send(self, msg):
        try:
            ...
            # Send the actual message in chunks to avoid buffer issues
            total_sent = 0
...
class Graph:
    ...
    def update_connectivity_matrix(self, rho=0.2):
        nbr_nodes = len(self.peers)
        def generate_connected_graph(nb_nodes, density):
            A = np.zeros((nb_nodes, nb_nodes))
            # Ensure connectivity by creating a chain
            for i in range(nb_nodes - 1):
                A[i][i + 1] = 1
                A[i + 1][i] = 1
            # Add additional edges based on rho
            for i in range(nb_nodes):
                for j in range(i + 1, nb_nodes):
                    if A[i][j] == 0 and np.random.rand() < density:
                        A[i][j] = 1
                        A[j][i] = 1
            # Set zeros on the diagonal
            np.fill_diagonal(A, 0)
            return A

        A = generate_connected_graph(nbr_nodes, rho)
        # Step 2: Initialize W with positive values where A[i][j] == 1
        W = np.zeros((nbr_nodes, nbr_nodes))
        for i in range(nbr_nodes):
            for j in range(nbr_nodes):
                if A[i][j] == 1:
                    W[i][j] = np.random.rand() + 0.1  # Ensure positive values greater than 0.1

        # Step 3: Apply Sinkhorn-Knopp algorithm with zeros
        tolerance = 1e-10
        ...
        for _ in range(max_iterations):
            ...
            for j in range(nbr_nodes):
                if col_sums[j] > 0:
                    W[:, j] /= col_sums[j]
            # Check convergence
            if np.allclose(W, W_prev, atol=tolerance):
                break
        # Ensure that W[i][i] == 0
